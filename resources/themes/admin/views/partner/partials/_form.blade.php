@include('partner.partials._buttons', ['class' => 'buttons-top'])
            ...
            <div class="tab-content">
                @foreach (config('app.locales') as $key => $locale)
                    <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_{!! $locale !!}">
                        @include('views.partner.tabs.locale', array('errors' => $errors, 'model' => $model , 'locale' => $locale))
                    </div>
                @endforeach

                <div class="tab-pane" id="general">
                    @include('partner.tabs.general')
                </div>
            </div>
        </div>
...
@include('partner.partials._buttons')