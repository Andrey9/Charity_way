@include('done.partials._buttons', ['class' => 'buttons-top'])
            ...
            <div class="tab-content">
                {{--@foreach (config('app.locales') as $key => $locale)
                    <div class="tab-pane fade in @if ($key == 0) active @endif" id="tab_{!! $locale !!}">
                        @include('views.done.tabs.locale', array('errors' => $errors, 'model' => $model , 'locale' => $locale))
                    </div>
                @endforeach--}}

                <div class="tab-pane active" id="general">
                    @include('done.tabs.general')
                </div>
            </div>
        </div>
...
@include('done.partials._buttons')