...
<html lang="en">
    ...
    <body class="skin-blue {!! !empty($body_css_class) ? $body_css_class : 'sidebar-mini' !!}">
        ...
        <script>
            $(document).on('click', '.add-current-product-to-list', function(event){
                var _id =$(this).data('id');
                var token = $(this).data('token');
                var _pos = $('.duplication').find('.duplication-row').length;
                console.log (_pos);
                $.ajax({
                    type: 'get',
                    url: '/admin/add_current_product',
                    data: {
                        id: _id,
                        _token: token,
                        pos: _pos
                    ...
                    var status = true;
                    $.each($('input[data-check="product_id"]'), function() {
                        if($(this).val() == _id) {
                            status = false;
                            return false;
                        }
                    ...
                    if(status) {
                        $('.duplication-row').last().after(response);
                        $('.find_products').val('');
                        $('.product-find-list').hide();
                    }
                });
            ...
            $('.find_products').on('keyup', function(event){
                var pr_list =$('.product-find-list');
                var _value = $(this).val();
                if (!_value.length){
                    pr_list.hide();
                }
